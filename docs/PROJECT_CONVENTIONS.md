...
1. **Centralized Factory Registry**: Manage and register all test fixtures in the centralized [page-factory.ts](file:///d:/Works/EMBRIX/Automation/EmbrixAuto/fixtures/page-factory.ts). Avoid splitting into multiple separate files to prevent import cycles and code fragmentation.
...
## 6. Dependency Injection Flow (DI Flow)

The test automation architecture in the project operates under a strict separation of concerns flow:

```
[Spec File (*.spec.ts)]
         │
         ▼ (Request fixture via parameters)
[Centralized Fixture (page-factory.ts)]
         │
         ├──► (Instantiate POM with Page context) ──► [Page Object (*.page.ts)] ──► UI Actions
         └──► (Instantiate Helper with Request)   ──► [Helper (*.helper.ts)]     ──► Call API / DB
```

* **Spec**: Only manages high-level test scenarios and assertion checkpoints (`expect`).
* **Fixture**: Coordinates and resolves dependencies, injecting ready-to-use Page Objects and Helpers into the Spec.
* **Page Object (POM)**: Manages UI locators and provides business interaction methods on screen.
* **Helper**: Provides non-UI operations (such as calling REST/GraphQL APIs or querying the DB directly) to quickly set up the test environment.

---

## 7. Helpers Conventions
...
## 8. Standard Timeouts & Anti-Flakiness Rules
...
2. **Soft Catching Mechanism for Loading Screen/Spinner**:
   - When writing locator wait statements for loading screen/spinner transitions (e.g. `waitForLoadingToDisappear()`), always attach an empty `.catch(() => {})` handler to both the visible and hidden wait steps.
   - This prevents tests from hard-crashing on fast pages where the spinner either never renders or disappears before Playwright evaluates the wait condition, allowing the execution flow to continue gracefully.
...
1. **Separation of Concerns**: Never execute SQL queries directly inside Page Objects or Spec files. All SQL operations must be abstracted into dedicated database helpers (e.g., `helpers/db/job-schedule.db.ts`, `helpers/db/provisioning.db.ts`).
...
## 10. Sidebar Component Convention
...
4. **Visibility as source of truth**: The component checks whether the target sub-menu items are already visible on the screen before clicking, avoiding unnecessary toggle actions. It relies on actual element visibility instead of stale properties (like classes or attributes) which can be unreliable in SPA routing.
...
*Document last updated: 2026-06-13 | Version 4.3 | Project: EmbrixAuto*
